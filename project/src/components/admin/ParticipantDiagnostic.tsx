...
interface ExamResultRow {
  ...
  review_snapshot: {
    questions: any[];
    answers: Record<string, { questionId: string; selectedAnswer: string | null; isMarked: boolean }>;
  } | null;
}
...
function computeDiagnostics(questions: any[], answers: any): CategoryDiag[] {
  // answers is Record<questionId, {questionId, selectedAnswer, isMarked}>
...
function ParticipantDetail({ participant, onBack }: { participant: Profile; onBack: () => void }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from('exam_results')
        .select('id, package_name, package_type, score_tiu, score_twk, score_tkp, total_score, passed, completed_at, review_snapshot')
  ...
  async function handleSelectResult(row: ExamResultRow) {
    ...
    const snapshot = row.review_snapshot;
    if (snapshot?.questions && snapshot.questions.length > 0) {
      setDiags(computeDiagnostics(snapshot.questions, snapshot.answers));
    }
        ...
        <div className="lg:col-span-2">
          {!selectedResult ? (
            ...
            <div className="flex flex-col items-center justify-center h-48 gap-2 text-gray-400 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-sm">Snapshot soal tidak tersedia untuk hasil ujian ini.</p>
            </div>
          ) : (
