...
import { UserCheck, UserX, RefreshCw, Shield, Clock, CheckCircle, Search } from 'lucide-react';
...
export default function ManageAdmins() {
  ...
  async function handleApprove(id: string) {
    ...
    await supabase
      ...
      .eq('id', id);
    setAdmins((prev) => prev.map((a) => a.id === id ? { ...a, is_approved: true } : a));
  ...
  async function handleRevoke(id: string) {
    ...
    await supabase
      ...
      .eq('id', id);
    setAdmins((prev) => prev.map((a) => a.id === id ? { ...a, is_approved: false } : a));
    setActionLoading(null);
  }
  ...
  const filtered = admins.filter((a) => {
    ...
    const matchFilter = filter === 'all' || (filter === 'pending' && !a.is_approved) || (filter === 'approved' && a.is_approved);
    return matchSearch && matchFilter;
  });

  const pendingCount = admins.filter((a) => !a.is_approved).length;
  const approvedCount = admins.filter((a) => a.is_approved).length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Setujui atau cabut akses akun Admin</p>
        </div>
        <button onClick={loadAdmins} className="text-gray-400 hover:text-gray-600 transition-colors">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Admin', value: admins.length, color: 'text-[#1e3a8a]', bg: 'bg-blue-50' },
          { label: 'Menunggu Persetujuan', value: pendingCount, color: 'text-amber-600', bg: 'bg-amber-50' },
          { label: 'Disetujui', value: approvedCount, color: 'text-[#10b981]', bg: 'bg-emerald-50' },
        ].map((s, i) => (
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((admin, i) => (
            ...
            >
              <div className="w-10 h-10 bg-[#1e3a8a]/10 rounded-xl flex items-center justify-center flex-shrink-0">
                <Shield className="w-5 h-5 text-[#1e3a8a]" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-800 truncate">{admin.full_name}</p>
                <p className="text-gray-500 text-sm truncate">{admin.email}</p>
                {admin.created_at && (
                  <p className="text-gray-400 text-xs mt-0.5 flex items-center gap-1">
                    <Clock className="w-3 h-3" />
                    Daftar {new Date(admin.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </p>
                )}
              </div>
                ...
                {admin.is_approved ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      disabled={actionLoading === admin.id}
                      className="flex items-center gap-1.5 text-xs font-semibold text-white bg-[#1e3a8a] hover:bg-[#1e40af] px-3 py-1.5 rounded-full transition-colors disabled:opacity-60"
                    >
