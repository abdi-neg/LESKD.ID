...
import { loadReviewSnapshot } from '../../lib/examPersistence';
  ...
  return (
    <div className="space-y-3">
      {records.map((record, i) => {
        const hasReview = !!loadReviewSnapshot(record.id);
        return (
          <motion.div
            key={record.id}
              ...
              <div className="flex flex-col gap-2 flex-shrink-0">
                {onViewDetails && (
                  <button
                    ...
                    className="px-3 py-2 rounded-lg bg-[#1e3a8a]/10 hover:bg-[#1e3a8a]/20 text-[#1e3a8a] text-xs font-semibold transition-colors flex items-center gap-1"
                  >
                    <Eye className="w-3.5 h-3.5" />
                    <span className="hidden sm:inline">Lihat</span>
                  </button>
                )}
                {onViewReview && hasReview && (
                  <button
                    onClick={() => onViewReview(record.id)}
                    className="px-3 py-2 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-semibold transition-colors flex items-center gap-1"
                  >
                    <BookOpen className="w-3.5 h-3.5" />
                    <span className="hidden sm:inline">Bahas</span>
                  </button>
                )}
              </div>
            </div>
